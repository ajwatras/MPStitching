...
class Stitcher:
	...
	def applyHomography(self,imageB,imageA,H):
		...
		if (x_bound+x_shift > SIZE_BOUNDS[0]) or (y_bound+y_shift > SIZE_BOUNDS[1]):
			...
			if (SIZE_BOUNDS[1] - imageB2.shape[0] > 0) and (SIZE_BOUNDS[0] - imageB2.shape[1] > 0):
				result1 = np.pad(imageB2,((0,SIZE_BOUNDS[1] - imageB2.shape[0]),(0,SIZE_BOUNDS[0] - imageB2.shape[1]),(0,0)),'constant', constant_values=0) 
			else:
			...
			#mask1 = detectMask(result1)
			#mask2 = detectMask(result2)
		...
		result2 = cv2.warpPerspective(imageA, np.dot(shift_H,H), (x_bound+x_shift,y_bound+y_shift))
